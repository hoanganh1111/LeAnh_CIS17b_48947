    ...
    <body>
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                if($idx == $row["id_shop_user"]){
                    $str='<form action="update_user.php" method="post">';
                    $str.='<br>User ID:<br><input type="text" name="id_shop_user" value="'.$row['id_shop_user'].'" readonly/></p>';
                    ...
                    $str.='Email:<br><input type="text" name="email_address" value="'.$row['email_address'].'" required/></p>';
