...
<html>
    ...
    <body>
        ...
        $sql = "SELECT  `user_id`, `first_name`, `last_name`, `email`, `username`, `password` FROM `DnD_48947`.`user_entity` AS `user_entity`";
        // access the database
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                if($idx == $row["user_id"]){
                    ...
                    $str.='Username:<br><input type="text" name="username" value="'.$row['username'].'" required/></p>';
