...
function Archer(str, int, dex, con){
...
Archer.prototype.sentToServer = function(){
    var y_loc = Math.floor((Math.random() * 4) + 1);
    var x_loc = Math.floor((Math.random() * 4) + 1);
    
    var url = "updateCharacter.php?class_id=5&str=" + this.STR + "&int=" + this.INT + "&dex=" + this.DEX + "&con=" + this.CON + "&hp=" + this.HP + "&ma=" + this.MA + "&def=" + this.DEF + "&atk=" + this.ATK + "&matk=" + this.MATK + "&spd=" + this.SPD + "&sight=" + this.SIGHT + "&x_loc=" +x_loc+ "&y_loc=" +y_loc+ "&inventory=" + this.inventory + "&level=" + this.LVL + "&exp=" + this.EXP;
    document.location.replace(url);
}

Archer.prototype.baseStat = function(){
    ...
    this.MATK  = 1 + 1*this.INT + 1*this.DEX;
    this.SPD   = 5 + 1*this.DEX;
    this.SIGHT = 4;
};

Archer.prototype.LevelStat = function(str, int, dex, con){
    this.STR += str;
    this.INT += int;
    this.DEX += dex;
    this.CON += con;
...
Archer.prototype.LevelSkill = function(exp){
...
Archer.prototype.Weapon = function(weapon){
    for(var i=0; i< 3; i++){
          this.equipment[0][i] = weapon[0][i];
    }
};

Archer.prototype.Top = function(top){
    for(var i=0; i< 3; i++){
          this.equipment[1][i] = top[1][i];
    }
};

Archer.prototype.Chest = function(chest){
    for(var i=0; i< 3; i++){
          this.equipment[2][i] = chest[2][i];
    }
};

Archer.prototype.Inventory = function(item){
