...
<html>
    ...
    <body>
        ...
        $sql = "SELECT `id_shop_user` FROM `ale_48947`.`entity_shop_user` AS `entity_shop_user`";
        ...
        // Define the query:
        $sql = "SELECT  `id_shop_user`, `first_name`, `last_name`, `email_address`, `password` FROM `ale_48947`.`entity_shop_user` AS `entity_shop_user` ORDER BY `".$sort."` LIMIT ".$start.", ".$display."";
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                ...
                $strRow.=('<td align="left"><a href="admin_edit.php?id_shop_user='.$row["id_shop_user"].'">Edit</a></td>');
                $strRow.=('<td align="left"><a onclick="deleteUser('.$row['id_shop_user'].')" style="cursor:pointer">Delete</a></td>');
                $strRow.=('<td align="left">'.$row['first_name'].'</td>');
                $strRow.=('<td align="left">'.$row['last_name'].'</td>');
                $strRow.=('<td align="left">'.$row['email_address'].'</td>');
        ...
        <script type="text/javascript">
            function deleteUser(userLoc){
                ...
                if(conf == true){
                    location.assign("delete_user.php?id_shop_user='"+userLoc+"'");
                }
            }
        </script>
