...
<html>
    ...
    <body>
        ...
        //LOCAL
//        include 'LOCAL.php';
//        $sql = "SELECT `first_name` AS `First Name`, `last_name` AS `Last Name`, `email_address` AS `Email`, `password` AS `Password` FROM `test`.`entity_admin` AS `entity_admin`";
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                if(strtolower($_POST["email"]) === strtolower($row["email_address"]) 
                    ...
                    echo "<script type=\"text/javascript\"> window.location.replace('admin_main.html')</script>";
                    $logedin = true;
                    }
                }
            }else {
