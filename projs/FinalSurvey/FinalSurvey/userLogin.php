...
<html>
    ...
    <body>
        ...
        $sql = "SELECT `first_name`, `last_name`, `email`, `password` FROM `ale_48947`.`entity_survey_user` AS `entity_survey_user`";
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                if(strtolower($_POST["email"]) === strtolower($row["email"]) 
                    ...
                    echo "<script type=\"text/javascript\"> window.location.replace('generateSurvey.html')</script>";
                    $logedin = true;
                    }
                }
            }else {
